...
// you can run both reverse & join at the same time. Instead it would look like:
// console.log(alphabetArray.reverse().join(''));
// reverse CHANGES the array - not returns a new array.
...
// if you do the following: console.log([array1.length-1]); it will use the length in terms of how many elements - which is 6.
// then subtracting 1 will leave your answer at 5.
array1.push(16);
array1.push(3);
console.log(array1);
console.log(array1[2]);
...
myToDoList.push('Homework', 'Wash Dishes', 'Take a Shower');
...
yourToDoList.push('Make the Bed', 'Water Plants');
...
function addingMachine (x){
       let sum = 0;
       for (let i = 0; i < x.length; i++){ // i is referencing the index #
              sum = sum + x[i];           // another way to write this line is sum += x[i];
       }
return sum;   // this is outside of the for loop, otherwise it would print out each time it iterates.
}
...
function reverse (x){
       if (typeof x === 'boolean'){
              ...
              return x.toString().split('').reverse().join(''); // you can't reverse a # so we converted to a string
